...
fun wrapIfNeedAndReThrow(e: TransportException) {
    ...
    if (e.status == TransportException.Status.NOT_AUTHORIZED || e.status == TransportException.Status.NOT_PERMITTED ||
        message.contains(JGitText.get().notAuthorized) || message.contains("Auth cancel") || message.contains("Auth fail") || message.contains(
            ": reject HostKey:"
        ) /* JSch */) {
...
val Repository.remoteBranch: String?
    get() = config.getString(
        ConfigConstants.CONFIG_REMOTE_SECTION,
        Constants.DEFAULT_REMOTE_NAME,
 ...
 * Save URL and Branch.
 */
fun Repository.setUpstream(url: String?, remoteBranchName: String = Constants.MASTER): StoredConfig {
    ...
    if (url.isNullOrEmpty()) {
        ...
        config.setString(
            ...
            '+' + Constants.R_HEADS + remoteBranchName + ':' + Constants.R_REMOTES + remoteName + '/' + localBranchName
        )
        ...
        config.setString(
            ...
            remoteName,
            "push",
            Constants.R_HEADS + localBranchName + ':' + Constants.R_HEADS + remoteBranchName
        );
        config.setString(
            ConfigConstants.CONFIG_REMOTE_SECTION,
            Constants.DEFAULT_REMOTE_NAME,
...
fun cloneBare(
    ...
    val result = repository.fetch(
        remoteConfig,
        if (credentialsStore == null) null else JGitCredentialsProvider(credentialsStore, repository),
        progressMonitor
    ) ?: return repository
    var head = findBranchToCheckout(result)
    if (head == null) {
        val branch = Constants.HEAD
        head = result.getAdvertisedRef(branch) ?: result.getAdvertisedRef(Constants.R_HEADS + branch) ?:
                result.getAdvertisedRef(Constants.R_TAGS + branch)
    }
    ...
    if (head.name.startsWith(Constants.R_HEADS)) {
        ...
        val autoSetupRebase =
            config.getString(ConfigConstants.CONFIG_BRANCH_SECTION, null, ConfigConstants.CONFIG_KEY_AUTOSETUPREBASE)
...
fun Repository.processChildren(
    path: String,

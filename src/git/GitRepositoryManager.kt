...
class GitRepositoryManager(private val credentialsStore: Lazy<IcsCredentialsStore>, dir: Path) :
    BaseRepositoryManager(dir), GitRepositoryClient {
    ...
    override fun getRemoteBranch(): String? = repository.remoteBranch
    ...
    override fun commit(indicator: ProgressIndicator?, syncType: SyncType?, fixStateIfCannotCommit: Boolean): Boolean {
        lock.write {
            try {
                ...
                return commitIfCan(
                    indicator,
                    if (!fixStateIfCannotCommit || syncType == SyncType.OVERWRITE_LOCAL) repository.repositoryState else repository.fixAndGetState()
                )
    ...
    override fun resetToMy(indicator: ProgressIndicator, localRepositoryInitializer: (() -> Unit)?): UpdateResult =
        Reset(this, indicator).reset(false, localRepositoryInitializer)
    ...
    fun renameDirectory(pairs: Map<String, String?>, commitMessage: String): Boolean {
        ...
        repository.commit(
            with(IdeaCommitMessageFormatter()) { StringBuilder().appendCommitOwnerInfo(true) }.append(
                commitMessage
            ).toString()
        )
        return true

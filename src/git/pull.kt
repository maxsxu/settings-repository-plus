...
class GitRepositoryClientImpl(
    ...
    private val credentialsStore: Lazy<IcsCredentialsStore>
) : GitRepositoryClient {
...
open internal class Pull(
    val manager: GitRepositoryClient,
    val indicator: ProgressIndicator?,
    val commitMessageFormatter: CommitMessageFormatter = IdeaCommitMessageFormatter()
) {
    ...
    fun fetch(
        ...
        val fetchResult =
            repository.fetch(remoteConfig, manager.credentialsProvider, indicator.asProgressMonitor()) ?: return null
        ...
        return fetchResult.getAdvertisedRef(config.getRemoteBranchFullName())
        ...
        try {
            ...
            if (headId == null) {
                ...
                return MergeResultEx(
                    ...
                    arrayOf(null, srcCommit),
                    ImmutableUpdateResult(dirCacheCheckout.updated.keys, dirCacheCheckout.removed)
                )
            }
            ...
            if (!forceMerge && revWalk.isMergedInto(srcCommit, headCommit)) {
                return MergeResultEx(
                    ...
                    arrayOf<ObjectId?>(headCommit, srcCommit),
                    EMPTY_UPDATE_RESULT
                )
            } else if (!forceMerge && fastForwardMode != FastForwardMode.NO_FF && revWalk.isMergedInto(
                    headCommit,
                    srcCommit
                )
            ) {
                ...
                dirCacheCheckout =
                        DirCacheCheckout(repository, headCommit.tree, repository.lockDirCache(), srcCommit.tree)
                dirCacheCheckout.setFailOnConflict(false)
                dirCacheCheckout.checkout()
                ...
                if (squash) {
                    mergeMessage = ""
                    repository.writeSquashCommitMsg(
                        SquashMessageFormatter().format(
                            RevWalkUtils.find(
                                revWalk,
                                srcCommit,
                                headCommit
                ...
                refLogMessage.append(
                    if (revWalk.isMergedInto(
                            headCommit,
                            srcCommit
                        )
                    ) "recursive" else mergeStrategy.name
                )
                refLogMessage.append('.')

                var result = if (merger is ResolveMerger) ImmutableUpdateResult(
                    merger.toBeCheckedOut.keys,
                    merger.toBeDeleted
                ) else null
                if (noProblems) {
                    ...
                    repository.writeMergeCommitMsg(
                        MergeMessageFormatter().formatWithConflicts(
                            mergeMessage,
                            unmergedPaths
                        )
                    )
                    return MergeResultEx(
                        ...
                        arrayOf(headCommit.id, srcCommit.id),
                        result!!,
                        lowLevelResults
                    )
                } else {
...
class MergeResultEx(
    ...
    val mergedCommits: Array<ObjectId?>,
    val result: ImmutableUpdateResult,
    val conflicts: Map<String, org.eclipse.jgit.merge.MergeResult<out Sequence>>? = null
)
...
private fun updateHead(refLogMessage: StringBuilder, newHeadId: ObjectId, oldHeadID: ObjectId, repository: Repository) {
    ...
    when (rc) {
        ...
        RefUpdate.Result.REJECTED, RefUpdate.Result.LOCK_FAILURE -> throw ConcurrentRefUpdateException(
            JGitText.get().couldNotLockHEAD,
            refUpdate.ref,
            rc
        )
        else -> throw JGitInternalException(
...
private fun resolveConflicts(
    mergeProvider: JGitMergeProvider<out Any>,
    unresolvedFiles: MutableList<VirtualFile>,
...
internal fun resolveUnmergedConflicts(repository: Repository) {
    ...
    repository.newObjectReader().use { reader ->
        ...
        for (i in 0..(dirCache.entryCount - 1)) {
            ...
            if (!entry.isMerged) {
                conflicts.getOrPut(entry.pathString, { arrayOfNulls<ByteArray>(3) })[entry.stage - 1] =
                        reader.open(entry.objectId, Constants.OBJ_BLOB).cachedBytes
            }
        }
    }

    resolveConflicts(
        JGitMergeProvider(repository, conflicts, { path, index -> get(path)!!.get(index) }),
        conflictsToVirtualFiles(conflicts),
        repository
    )
}
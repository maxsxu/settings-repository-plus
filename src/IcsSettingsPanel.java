...
public class IcsSettingsPanel extends DialogWrapper {
    ...
    public IcsSettingsPanel(@Nullable Project project) {
        ...
        branchTextField.setText(IcsManagerKt.getIcsManager().getRepositoryManager().getRemoteBranch());

        syncActions = UpstreamEditorKt.createMergeActions(project, urlTextField, branchTextField, getRootPane(), new Function0<Unit>() {
            @Override
            public Unit invoke() {
                doOKAction();
                return null;

...
fun createMergeActions(
    project: Project?,
    urlTextField: TextFieldWithBrowseButton,
    branchTextField: JTextField,
    ...
    return Array(3) {
        ...
        object :
            AbstractAction(icsMessage("action.${if (syncType == SyncType.MERGE) "Merge" else (if (syncType == SyncType.OVERWRITE_LOCAL) "ResetToTheirs" else "ResetToMy")}Settings.text")) {
            private fun saveRemoteRepositoryUrl(): Boolean {
                val url = urlTextField.text.nullize(true)
                val branch = branchTextField.text.nullize(true)
                ...
                // TODO: set branch
                repositoryManager.setUpstream(url, branch)
                return true
            }
            ...
            override fun actionPerformed(event: ActionEvent) {
                ...
                try {
                    ...
                    if (!upstreamSet || e is NoRemoteRepositoryException) {
                        Messages.showErrorDialog(
                            ...
                            icsMessage("set.upstream.failed.title")
                        )
                    } else {
                        Messages.showErrorDialog(
                            dialogParent,
                            StringUtil.notNullize(e.message, "Internal error"),
                            icsMessage(if (e is AuthenticationException) "sync.not.authorized.title" else "sync.rejected.title")
                        )
                    }
                    return
                }
                ...
                NOTIFICATION_GROUP.createNotification(icsMessage("sync.done.message"), NotificationType.INFORMATION)
                    .notify(project)
                okAction()
            }
        }
